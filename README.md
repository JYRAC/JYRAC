この index.html を事務局メンバーや開発チームがスムーズに運用・管理できるようにするための解説書（README.md）を作成しました。

このテキストを README.md という名前のファイルとして保存し、ソースコードと同じフォルダに置いておくと、今後の引き継ぎや更新作業の際に大変便利です。

📝 JAYRAC（日本若者防災復興協会）公式サイト 運用マニュアル
本リポジトリは、特定非営利活動法人 日本若者防災復興協会（JAYRAC）のコーポレートサイトのソースコードです。
学校や官公庁のサイトをベースにした、社会的信用度を高める2カラム（左右分割）レイアウトを採用しています。

HTML、CSS、JavaScriptが1つのファイル（index.html）にすべて集約されているため、高度なサーバー環境やCMSを使わず、手軽に更新・管理が可能です。
